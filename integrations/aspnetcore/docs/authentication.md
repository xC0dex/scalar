...
To ensure that the necessary authentication schemes are included in the OpenAPI document, you need to implement an `OpenApiDocumentTransformer`. This allows you to add the required security schemes to the OpenAPI document, ensuring that Scalar.AspNetCore can recognize and display them appropriately. It is also possible to provide multiple authentication schemes.
...
To set up Bearer authentication in your API, start by configuring the authentication scheme in the DI container:
...
Next, add this security scheme to the OpenAPI document so that Scalar can recognize and use it. This can be achieved by creating a custom `OpenApiDocumentTransformer`:
...
OAuth is a widely used authorization standard that typically uses a Bearer Token to authenticate users. It supports various flows, including Authorization Code, Implicit and Client Credentials. The appropriate flow depends on the specific requirements and capabilities of your identity provider.
...
To set up OAuth authentication in your API, start by configuring the authentication scheme in the DI container:
