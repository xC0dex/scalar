...
This .NET package `Scalar.Aspire` provides an easy way to render beautiful API references based on OpenAPI/Swagger documents in .NET Aspire applications.
...
You can customize your Scalar integration with the following options:

```csharp
builder.AddScalarApiReference(options =>
{
    options.Title = "My Custom API Documentation";
});
```

## Multiple OpenAPI Documents

If your API exposes multiple OpenAPI documents (e.g., for different API versions), you can configure them in your Aspire host:

```csharp
scalar.WithReference(api, options => 
{
